...
class ColorDetector(BaseDetector):
    ...
    def detect(path, visualize: bool = False) -> Union[List[Centroid], np.ndarray]:
        ...
        x_mean, y_mean = cv.findNonZero(cv.cvtColor(result1,cv.COLOR_RGB2GRAY)).squeeze().mean(0)
        if cv.findNonZero(cv.cvtColor(result2,cv.COLOR_RGB2GRAY)) is None:
            x_mean_hole, y_mean_hole = (0,0)
        else:
            x_mean_hole, y_mean_hole = cv.findNonZero(cv.cvtColor(result2,cv.COLOR_RGB2GRAY)).squeeze().mean(0)
